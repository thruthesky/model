...
|---|---|---|
| `--height` | 1.8 | 목표 키(m) |
| **`--triangles`** | **4800** ⚠️ | 🛑 **언제나 `0` 을 명시한다.** 사람이 삼각형 수를 지시했을 때만 그 값을 준다. 아래 경고 참조 |
...
>
> **`--triangles 0` 을 언제나 명시한다.** 사람이 숫자를 지시했을 때만 그 값을 준다.
>
> 🔧 **사람이 할 일** — 스크립트의 `DEFAULT_TRIANGLES = 4800` 을 **`0`** 으로 바꾸면
> 이 함정 자체가 사라진다. 코드 수정은 사람이 한다(CLAUDE.md).
