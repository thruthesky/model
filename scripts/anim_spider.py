...
def make_walk():
    n = LEN["walk"]
    act = new_action("walk")
    stride = SPAN * 0.52          # 앞뒤 보폭(스프라이트에서 보이려면 과장해야 한다)
    lift = SPAN * 0.34            # 스윙 때 드는 높이
    for f in range(n + 1):
        t = f / n
        for leg in legs:
            ph = (t + (0.5 if leg["grp"] else 0.0)) % 1.0
            fwd = Vector((0, 1, 0))                     # 진행 방향(+Y)
        ...
        bob = math.sin(t * 4 * math.pi) * SPAN * 0.055
        sway = math.sin(t * 2 * math.pi) * SPAN * 0.040
...
def make_attack():
    ...
    # 진행 방향(+Y) 쪽 다리 2개 = 앞다리
    front = sorted(legs, key=lambda l: -l["rest"].y)[:2]
    for f in range(n + 1):
        t = f / n
        if t < 0.30:                       # 준비: 몸을 낮추고 뒤로 당긴다
            s = t / 0.30
            body_z, body_y, pitch = -SPAN * 0.16 * s, -SPAN * 0.13 * s, 0.26 * s
            rear = s
        elif t < 0.52:                     # 타격: 앞으로 튀어나가며 내려찍는다
            s = (t - 0.30) / 0.22
            body_z = -SPAN * 0.16 + SPAN * 0.30 * s
            body_y = -SPAN * 0.13 + SPAN * 0.42 * s
            pitch = 0.26 - 0.72 * s
            rear = 1.0 - s
        else:                              # 복귀
            s = (t - 0.52) / 0.48
            body_z = SPAN * 0.14 * (1 - s)
            body_y = SPAN * 0.29 * (1 - s)
            pitch = -0.46 * (1 - s)
            rear = 0.0
        set_body(loc=(0, body_y, body_z), rot=(pitch, 0, 0))
        for leg in legs:
            if leg in front:
                # 앞다리: 크게 들어올렸다가 내려친다
                if t < 0.30:
                    off = Vector((0, 0, SPAN * 0.60 * rear)) + leg["dir"] * (-SPAN * 0.14 * rear)
                elif t < 0.52:
                    s2 = (t - 0.30) / 0.22
                    off = Vector((0, 0, SPAN * 0.60 * (1 - s2))) + leg["dir"] * (SPAN * 0.26 * s2)
                else:
                    s2 = (t - 0.52) / 0.48
                    off = leg["dir"] * (SPAN * 0.26 * (1 - s2))
            else:
