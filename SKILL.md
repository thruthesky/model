---
name: model
description: 라리엔의 액터(플레이어·몬스터·NPC)와 정적 기물(나무·풀·바위·벽·펜스·건물) 3D 모델을 생성·변환·수정하고 Godot용 GLB와 씬으로 검증한다. 기물의 삼각형 감축·리토폴로지·albedo 베이크·패턴/팔레트 재사용·번들 및 CPU/GPU 메모리 최소화 요청에 사용한다. 액터는 Tripo3D 생성, Auto-Rig Pro 리깅, Mixamo 애니메이션, 본 감축, 자세·축·원점·스케일 교정을 다룬다. Godot 정적 기물은 references/godot-props.md, 액터는 references/godot-pipeline.md로 분기한다. Flutter/Flame 2.5D용 16방향 스프라이트 아틀라스 베이크는 본문의 Phase B를 따른다.
---

# model — 액터·정적 기물 제작과 변환

이 파일은 Claude Code와 [Codex 진입점](../../../.agents/skills/model/SKILL.md)이 함께 사용하는 제작 절차의 정본이다.
작업 전에 프로젝트의 `AGENTS.md`, [SSOT](../game/references/SSOT.md), [CLAUDE.md](../../../CLAUDE.md)를 읽는다.
**프로젝트 SSOT와 사용자의 명시적 지시가 이 스킬의 과거 설명보다 우선한다.**

## 정적 기물은 먼저 이 분기로 간다

**나무·풀·바위·벽·펜스·건물 등 맵 기물의 생성·다운로드 모델 변환·경량화는
[godot-props.md](references/godot-props.md)를 반드시 읽고 수행한다.** 액터의 리깅 절차를 먼저 적용하지 않는다.

- **기물은 번들 사이즈와 CPU·GPU 메모리를 절대적으로 최소화한다**([SSOT §3.1](../game/references/SSOT.md#prop-resource-budget)). 필요한 실루엣·식별성·충돌·저사양 성능을 유지하면서 삼각형과 최종 정점 수를 함께 줄인다.
- 기물은 감축·리토폴로지·잎 카드화를 수행한다. 아래 액터용 **“다운로드 후 삼각형을 건드리지 않는다”** 규칙과 1,600삼각형 기본값을 기물에 적용하지 않는다. 기물 최적화에 별도의 숫자 지시를 다시 요구하지 않는다.
- **반복 표면을 미리 albedo에 베이크하고 대표 패턴·팔레트·UV를 재사용한다.** 베이크 자체가 용량을 줄이지는 않는다. 런타임 텍스처 해상도·고유 이미지·정점 속성을 실제로 줄여야 한다. **1K·2K는 기물별 기본값이나 최소값이 아니다.**
- **공유 `UNSHADED` 재질 1개**와 최소한의 메시·텍스처를 사용한다. 사용하지 않는 탄젠트·UV2·PBR 맵·그림자 메시·리그·애니메이션을 제거하고, 동일 기물 인스턴스의 리소스를 공유한다.
- 원본·고해상도 베이크·재생성 자료는 `game-assets/`의 `.gdignore` 아래, 최종 씬·GLB·재질·텍스처는 **사용자가 지정한 기물 폴더 안에 함께** 둔다. 자연어로 지정한 출력 경로도 유효하다.
- 완료 전 **실제 export 패키지 데이터, CPU·GPU 할당, 추가 인스턴스 비용, 모바일 화면**을 확인한다. 약 100KB는 나무·펜스의 사례 목표이며 모든 기물의 공통 상한이 아니다. 정확한 설정·측정 범위·재생성 경로는 공통 문서에 둔다.

## 🛑 액터·무기 외형의 품질 원칙 — **AAA 급 모델 디자인**

> **"제가 원하는 것은 오직 AAA 급 모델 디자인입니다."** — 원저자 2026-08-07

**액터·무기의 식별 가능한 외형을 조잡한 도형 조합으로 대체하지 않는다.** 아래는 그 품질 기준이다.
정적 맵 기물은 위의 용량 최소화 규칙과 전용 파이프라인을 따르며, 원본의 미세 무늬를 유지하려고 런타임 비용을 늘리지 않는다.
...
| **무기 · 장비**(리그 없는 장착물) | ② 생성 → ③ 다운로드 → **리깅 없이 GLB 로 변환**(④~⑦ 불필요) |
| **정적 맵 기물·소품**(나무·바위·펜스 등) | **[기물 파이프라인](references/godot-props.md)** — 원본 분석 → 형상 감축 → 패턴·팔레트 베이크 → 경량 GLB·공유 재질·충돌 씬 → 번들·메모리 검증 |
| 몬스터(비인간형 — 비행체·차량형) | ② 생성 → ③ 다운로드 → 리깅 없이 GLB. 움직임은 **노드 변환**으로 준다 |
| 지형 · 건물 · 배경 구조물 | 원시 도형을 **써도 된다** — 아래 예외 참조. 정적 기물의 최종 결과는 기물 파이프라인으로 최적화·검증 |
...
- ✅ **정적 기물의 원본 실루엣을 보존하는 리토폴로지** — 펜스 격자·기둥을 원본에서 측정해 적은 면으로 재구성하거나, 잎의 위치·크기에 맞춰 베이크 카드를 만드는 작업
...
| 대상 | 산출물 | 문서 |
|---|---|---|
| **Godot 3D 정적 맵 기물** | 경량 `.glb` + 공유 재질·텍스처 + 충돌 포함 `.tscn` | **[references/godot-props.md](references/godot-props.md)** |
| **Godot 3D 액터·장착 장비** | **`<NAME>.glb`** — Godot 에 바로 넣는다 | 🛑 **[references/godot-pipeline.md](references/godot-pipeline.md)** |
| ↳ 그 GLB 를 **씬에 올려 움직이기** | 데모 씬 + **임포트 해부·애니 원리**(4.7.2 실측) + **❓질문으로 찾기** | **[references/godot-scene.md](references/godot-scene.md)** |
...
**프로젝트가 Godot 이면 위 표에서 기물 또는 액터 문서로 간다.**
아래 ①~⑨ 는 2.5D 애니메이션 스프라이트 아틀라스를 굽는 경로이고, Godot 에서는 ⑧⑨ 를 쓰지 않는다.
기물의 albedo·패턴·팔레트 베이크는 별개이며 Godot에서도 사용한다.

## `/model` 명령 사용법 (Godot 액터 경로)

```
/model --bones 16 --kind human --animations <폴더> "<프롬프트>"
```

🛑 **`--triangles` 는 기본 사용법에 없다.** 폴리곤은 Tripo3D 에서 받은 그대로 쓴다.

| 옵션 | 값 | 기본 | 뜻 |
|---|---|---|---|
| `--bones` | **16** · 25 | **16** | 본 예산. 저사양 우선이라 **최저 16 이 기본** |
| `--kind` | **human** · animal · drone · prop | `human` | 형태 → 리깅 경로 |
| **`--triangles`** | 1600 · 3200 · 4800 · 6400 · 7200 | 🛑 **없음 — 조절 안 함** | 🛑 **사람이 명시할 때만 준다.** 그때 Blender Decimate 가 돈다(아래 절) |
...
| ⭕ 예외 | 사람이 `--output` 또는 **자연어로 명시한 경로** | 그 경로에 쓴다. 기물은 원본과 런타임 자료를 분리한다 |
...
이유가 되지 않는다. **출력 경로 지시가 없으면 `./outputs` 이다.**
“`maps/props/walls/green_fence`에 저장”도 명시적 출력 지시다. 그 폴더 안에 `.tscn`을 포함한 최종 결과를 함께 두며, 같은 이름의 폴더를 한 번 더 만들지 않는다.
...
① 로그인 → ② 생성(T-Pose) → ③ 리깅 없이 다운로드
  ...
  → ⑤-T 🛑 **T-포즈가 아니면 T-포즈로 정렬**(human 필수 · ②-T)
...
### 🛑🛑 ②-T `human` 형은 **반드시 T-포즈여야 한다** — 아니면 T-포즈로 바꾼다 (ABSOLUTE)

> # 인간형(`--kind human`) 모델이 T-포즈가 아니면, **리깅 뒤 반드시 T-포즈로 자세를 바꾼다.**
>
> 〔원저자 지시 2026-09-04〕 **선택이 아니다.** A-포즈·휴식 포즈·웅크린 자세로
> 받은 모델을 그대로 진행하면 **애니메이션이 예외 없이 무너진다.**

**왜 — 리타게팅은 rest 자세를 "보존"하기 때문이다.** 보정식이

    (src_pose @ src_rest⁻¹) @ tgt_rest

라서, Mixamo 에서 가져오는 것은 **rest 대비 회전차**뿐이고 기준 자세는 **우리 리그의
rest** 다. Mixamo 는 **T-포즈 rest** 를 전제하므로, rest 가 A-포즈면 *"T-포즈에서 팔을
75° 내리는"* 회전차가 **이미 내려간 팔에 또** 더해져 팔이 몸통을 관통한다.
**②-V 의 벌어진 다리와 완전히 같은 구조의 문제**이고, 크기만 훨씬 크다.

🛑 **리깅·검증을 전부 통과해도 걸러지지 않는다.** 실측(2026-09-04 `scrap` 메카)에서
⑥ 검증은 **22/22 역할 · 교집합 52 · 정점 그룹 전부 일치**로 통과했는데 애니는 전부
뒤틀렸다. 원인은 오직 rest 각도차였다:

| 본 | rest 각도차 | 타겟 방향 | Mixamo 방향 |
|---|---|---|---|
| `LeftArm` | **70.2°** | (0.34, 0.18, **−0.92**) 아래 | (1, 0, 0) 수평 |
| `LeftForeArm` | **83.1°** | | |
| `LeftHand` | **86.3°** | | |
| `Spine` · `Neck` | 21.3° · 19.0° | | |

**판정한다 — 눈이 아니라 각도로**

```bash
blender --background --python .claude/skills/model/scripts/align_tpose.py -- \
  <리깅된.blend|.fbx> game-assets/animations/default/idle.fbx --check-only
echo "종료코드=$?"    # 0 = T-포즈 · 1 = 아니다
```

**고친다 — `align_tpose.py` 가 리그와 메시를 함께 T-포즈로 굳힌다**
...
| | |
|---|---|
| **언제** | 🛑 **리깅(④⑤) 뒤 · 리타게팅(⑦) 앞.** 리깅 전엔 본이 없어 자세를 못 바꾸고, 리타게팅 뒤엔 액션이 이미 구워져 늦다 |
| **무엇을** | 팔(Shoulder·Arm·ForeArm·Hand·손가락) + 척추·목·머리를 Mixamo rest 에 맞춘다 |
| 🛑 **다리는 기본 제외** | T-포즈는 무릎을 곧게 편다. **각진 기계 부품·굽은 다리 모델은 그 과정에서 정강이·발 메시가 늘어나 찢어진다**(실측). 다리 각도차 13~17° 는 리타게팅이 감내한다. 꼭 필요하면 `--parts all` 을 주되 **반드시 렌더해서 확인한다** |
| **메시도 함께** | 리그 rest 만 바꾸면 스킨이 통째로 어긋난다. 모디파이어를 복제·적용해 포즈를 메시에 굽고 `armature_apply()` 로 rest 를 굳힌다 |

**메시가 T-포즈로 굳는 것은 문제가 아니다** — 게임에서는 언제나 애니메이션 자세가
재생된다. T-포즈로 보이는 것은 `RESET` 액션뿐이고, 그것이 Godot 블렌딩의 **올바른
기준 포즈**다.

> ✅ **가장 싼 해결은 애초에 T-포즈로 생성하는 것이다** — ② 의 `T-Pose` 토글 ON +
> 프롬프트의 `standing in T-pose with both arms straight out horizontally`.
> 🛑 다만 **토글은 팔만 편다** — 다리는 ②-V 가, 척추·목은 이 절이 따로 책임진다.

#### 🛑🛑 그러나 **사후 정렬이 항상 이기지는 않는다** — 반드시 A/B 로 비교한다

**정렬은 메시를 T-포즈로 "구워서" 새 rest 로 만든다.** 그 굽는 과정에서 팔·어깨의
파츠가 크게 이동하는데, **각진 기계 파츠나 분리된 장갑은 그때 손상**되고 그 손상된
형상이 rest 로 굳는다. 애니를 재생하면 왜곡이 그 위에 누적된다.

**실측 (2026-09-04 `scrap`)** — 같은 리그·같은 애니로 둘을 굽고 Godot 에서 비교했다:

| | rest 각도차 | Godot 애니 재생 |
|---|---|---|
| **T-포즈 정렬 후** | 107.7° → **39.0°** ✅ | 🛑 어깨 장갑이 몸에서 떨어져 나가고 형상이 흩어졌다 |
| **A-포즈 그대로** | 107.7° (그대로) | ✅ **몸통·머리·다리·장갑 전부 온전** |

**그래서 초기 실패의 진짜 원인은 rest 자세가 아니었다** — `retarget_to_arp_rig.py` 의
**Hips 이동을 아마추어 로컬 공간에서 재던 버그**였다(Mixamo 로컬은 Y-up 이라 전진이
상방으로 들어가 walk 에서 발이 **+0.814 m** 떠올랐다). 그것을 월드 기준으로 고치자
**A-포즈 그대로도 발 접지가 +0.025 m 로 정상**이 됐다.

🛑 **판단 순서**

1. **애니가 무너지면 먼저 Hips·스케일 계산이 월드 기준인지 확인한다.** rest 를 의심하기 전에.
2. rest 각도차가 크면 정렬을 **시도**하되, **정렬본과 원본을 둘 다 굽고 Godot 에서 비교**한다.
3. **유기체·부드러운 메시는 정렬이 거의 항상 이긴다. 각진 기계·분리된 장갑 파츠는 반대일 수 있다.**
4. 어느 쪽이든 **애초에 T-포즈로 생성**하는 것이 두 문제를 다 없앤다 — 위 ✅ 항목.
...
| 항목 | 값 | 왜 |
|---|---|---|
| **Ultra Mesh Quality** | 🛑 **끈다** | 폴리곤만 늘고 크레딧을 더 쓴다. 폴리곤 수는 `Polycount` 로 정한다 |
| **Texture Quality** | **2K** | 8K 는 ZIP 이 지연되거나 아예 오지 않는다(크레딧만 차감). Godot 경로는 로컬에서 다시 1024 로 줄인다 |
| **Topology** | 🛑 **`Triangle`** | `Quad` 로 두면 엔진이 다시 쪼개고 그 결과 수를 통제하지 못한다 |
| **Polycount** | 🛑 **`1600`** (기본) | **삼각형은 여기서 정한다.** Blender 로 깎지 않는다 — 아래 절 |

### 🛑🛑 폴리곤은 **Tripo3D 에서 받은 그대로 쓴다 — 기본은 조절하지 않는다**

> ### 🛑 다운로드한 메시의 삼각형 수에 **손대지 않는다.**
>
> **폴리곤은 Tripo3D 생성 단계에서 `Polycount` 로 정하고(기본 1600), 그 뒤로는
> 파이프라인 어디에서도 건드리지 않는다.** 로컬에서 깎아 만든 1600 은 같은 숫자가
> 나와도 **같은 물건이 아니다.**
>
> **예외는 하나뿐이다 — 사람이 `--triangles N` 을 명시적으로 지시했을 때.**
> 그때만 Blender Decimate 로 N 까지 조절한다. **Claude 가 스스로 판단해 깎지 않는다.**
>
> 🛑 **최고의 모델 디자인은 Tripo3D 가 1600 을 목표로 직접 생성했을 때만 나온다.**
> 원저자 지시(2026-09-02)로 확정됐다. 이 문서의 다른 서술과 충돌하면 **이 절이 맞다.**
> 제0원칙("오직 AAA 급 모델 디자인")이 요구하는 품질이 여기에 걸려 있다.

**세 가지 경우 — 이것이 전부다**

| 상황 | 무엇을 하는가 |
|---|---|
| **기본 (`--triangles` 없음)** | 🛑 **아무것도 하지 않는다.** `normalize_for_godot.py --triangles 0` |
| **Tripo3D 생성 단계** | ✅ `Geometry & Texture` 의 **`Polycount`** 로 정한다. **기본 1600** |
| **사람이 `--triangles N` 을 지시** | ⭕ 그때만 **Blender Decimate** 로 N 까지 조절하고 결과를 눈으로 확인해 보고한다 |

🛑 **"폴리곤이 많아 보여서" · "예산을 맞추려고" 스스로 깎지 않는다.** 사람이 숫자를
말하지 않았으면 조절 자체가 작업 범위 밖이다.

**UI 경로** — `Generate Model` 패널 → `General Settings` → **`Geometry & Texture`**:

| 항목 | 값 |
|---|---|
| **`Topology`** | 🛑 **`Triangle`** (옆의 `Quad` 를 고르지 않는다) |
| **`Polycount`** | 🛑 **`1600`** — 슬라이더 500 ~ 1,000,000, 오른쪽 숫자 상자에 직접 입력 |

지정하면 뷰포트 우상단에 **`Faces 1600 / 1600`** 으로 표시된다. 그 숫자를 확인하고 넘어간다.
**크레딧은 `Polycount` 값과 무관하다** — `Smart Mesh`·`Retopo` 처럼 추가 과금되는 기능이 아니다.

**왜 손대지 않는가 — 같은 1600 이라도 만드는 방법이 다르면 결과가 다르다**

| | **Tripo3D `Polycount = 1600`** ✅ | Blender Decimate → 1600 🛑 |
|---|---|---|
| 하는 일 | **1600 을 목표로 토폴로지를 만든다** | 이미 만들어진 100만 메시의 **에지를 무너뜨린다** |
| 얼굴·손·장비 디테일 | 예산을 우선 배분해 **형태가 남는다** | 면적 비례로 깎여 **뭉개진다** |
| 부속(무기·머리카락) | 생성 단계에서 함께 계산된다 | **폴리곤이 많은 쪽이 예산을 독식한다** |

**실측 (2026-09-02 `claudy`)** — `Polycount 1600` 으로 생성하고 Decimate 를 **한 번도
돌리지 않았다.** 최종 GLB 삼각형 1,600 · 정점 1,374 · 본 16 · 1.6 MB 로 얼굴·장갑·부츠의
형태가 그대로 남았다. 대조군은 `male` 이다 — Decimate 로 4,800 을 맞췄는데 **검이 95% 를
먹고 캐릭터가 234 삼각형**이 됐다. 총합은 예산 안이었지만 사람 형체가 나오지 않았다.

**그래서 `normalize_for_godot.py` 에는 `--triangles 0` 을 준다.** 정규화가 하는 일은
키 1.8m · 발바닥 원점 · Z-up · scale 1 뿐이고, **폴리곤은 그대로 남는다.**

⚠️ **옵션을 생략하면 안 된다** — 스크립트의 `DEFAULT_TRIANGLES` 가 아직 4800 이라
빼면 Decimate 가 조용히 돈다. **`0` 을 명시해야 "조절하지 않는다" 가 지켜진다.**

🛑 **100만 폴리곤 과거 자산을 발견해도 스스로 깎지 않는다.** 사람에게 알리고 판단을 받는다.

상세·근거는 [godot-pipeline.md](references/godot-pipeline.md) 의
「삼각형은 Tripo3D 에서 정한다」 절이 정본이다.

⚠️ **2.5D 아틀라스(Phase B) 경로는 폴리곤 예산이 헐겁다.** 128px 셀에 렌더할 뿐이라
페이스 수가 렌더 시간에만 영향을 준다. 그래도 **생성 단계에서 `Polycount` 로 정하는 규칙은
같다** — Blender 뷰포트 조작이 느려지는 것도, 형상이 뭉개지는 것도 같은 이유로 피한다.
...
**Tripo3D 의 Export 에는 Godot 옵션이 없다.** 목록에 있는 것은 GLB · FBX · OBJ · STL
정도이고, **Godot 은 glTF(`.glb`) 를 1급으로 지원**하므로 GLB 가 곧 Godot 용이다.

| 우선순위 | 포맷 | 언제 |
|---|---|---|
| **1순위** | **`GLB`** | **기본값. 가능하면 항상 이것** |
...
| | GLB | FBX |
|---|---|---|
| 텍스처 | **파일 안에 임베드** — 경로가 깨질 수 없다 | `.fbm` **별도 폴더**. 이름이 어긋나면 **회색 모델**이 된다 |
| 축 규약 | glTF 표준 **Y-up 고정** — 예측 가능 | 프리셋마다 다르다(`Mixamo`/`3dsmax` 는 축이 바뀐다) |
| Godot | **네이티브 1급 지원** | 임포터를 거친다 |
| 이 스킬 | `normalize_for_godot.py` 가 그대로 받는다 | 같이 받지만 `.fbm` 을 함께 옮겨야 한다 |

🛑 **실측 근거** — `male` 캐릭터를 FBX 로 받았을 때 `.fbm` 텍스처 폴더 경로가 어긋나
`embedding file … failed` 가 났고, 그 상태로 굽자 **텍스처 없는 회색 결과**가 나왔다.
GLB 였다면 구조적으로 발생할 수 없는 사고다.

⚠️ **GLB 를 쓴다고 정규화를 건너뛰지 않는다.** GLB 도 Tripo 는 **1.0 단위 정규화 메시**로
주므로 키 1.8m·발바닥 원점은 여전히 맞춰야 한다 → ② `normalize_for_godot.py`.
...
| **Format** | **`GLB`** ← 기본. 안 되면 `FBX` + 프리셋 `Blender` |
...
1. **임포트·정리**
   ...
   - 🛑 **페이스가 많아도 여기서 `Decimate` 로 깎지 않는다** — 삼각형은 ② 의 Tripo3D
     `Polycount` 에서 이미 정해져 있다. 100만 폴리곤 과거 자산을 구제할 때만 예외로 쓴다
...
| 🛑 **⑥ 검증을 전부 통과했는데 애니가 통째로 뒤틀린다 · 팔이 몸통을 관통한다** | **모델이 T-포즈가 아니다**(②-T). ⑥ 은 본 *이름*만 보고 rest *자세*는 보지 않아 22/22 로 통과시킨다. `align_tpose.py --check-only` 로 따로 판정하고 정렬한다 |
| **T-포즈 정렬을 했더니 정강이·발이 찢어졌다** | `--parts all` 로 다리까지 폈다. 각진 기계 부품은 무릎을 펴면 메시가 늘어난다 — 기본값 `arms+spine` 를 쓴다(②-T) |
| **걷기에서 캐릭터가 공중에 뜬다**(발이 수십 cm 위) | Hips 이동을 **아마추어 로컬 공간**에서 쟀다. Mixamo 리그의 로컬은 **Y-up** 이라 전방 이동이 상방으로 들어간다. **월드 공간**에서 재고 다리 길이 비도 월드로 잰다 |
| **`marker hand` 가 메시 bbox 밖으로 나온다 · 팔 본이 10cm 짜리로 생긴다** | 어깨가 넓어 팔 감지 임계(`몸통폭 × 1.8`)를 못 넘었다. 마커를 실측해 직접 준다(godot-pipeline.md ③) |
| **`Could not find shoulder, marker out of mesh?`** | ARP 는 마커의 **(x, z)에서 Y축 레이캐스트**로 판정한다 — 마커가 **정면 실루엣 안**이어야 한다. 몸통과 팔 사이가 빈 메카는 그 틈에 마커가 떨어진다 |
